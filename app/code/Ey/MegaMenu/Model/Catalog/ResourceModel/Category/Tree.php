...
class Tree extends \Magento\Catalog\Model\ResourceModel\Category\Tree
    ...
    )
    {
        ...
        $collection->addAttributeToSelect(
            array(
                'short_name', 'megamenu_activate', 'megamenu_image',
                'megamenu_image_url', 'megamenu_html', 'megamenu_static_block'
            )
        );
        ...
        if ($toLoad) {
            ...
            foreach ($collection as $category) {
                if ($this->getNodeById($category->getId())) {
                    ...
                    /** Add URL */
                    $this->getNodeById($category->getId())->setUrl($category->getUrl());
                }
            }
